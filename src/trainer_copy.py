...
class Trainer(transformers.Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps, learning_rate=None):
        """
    ...
    def meta_st(self, model, un_inputs, sampling_step=1, epsilon=1e-6, learning_rate=None, use_soft_label=True):
        ...
        for i in range(sampling_step):
            self.clear_memory()
            try:
                meta_inputs = next(self.meta_iter)
            except:
                self.meta_iter = self.meta_dataloader.__iter__()
                meta_inputs = next(self.meta_iter)
            ...
            with higher.innerloop_ctx(
                ...
                # if self.args.fp16 and _use_native_amp:
                #     new_loss = self.scaler.scale(new_loss)
                # elif self.args.fp16 and _use_apex:
                #     with amp.scale_loss(new_loss, diffopt) as scaled_loss:
                #         new_loss = scaled_loss
    ...
    def pseudo_data_selection(self, model, un_inputs, un_meta, learning_rate=None, use_soft_label=None):
        ...
        elif self.args.psuedo_selection_opt == "confidence":
    ...
    def re_init(self):
        ...
        model = model_fn(config, self.teacher_model.model_args, self.teacher_model.data_args)
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if self.args.is_semi == 1:
            self_training_total_steps = self.args.update_teacher_steps + finetune_teacher_steps
        ...
        self.meta_dataloader = self.get_meta_train_dataloader()
        ...
        # model, self.optimizer, self.train_dataloader,  self.meta_dataloader, self.un_train_dataloader, self.demo_train_dataloader\
        #     = self.accelerator.prepare(
        #     self.model, self.optimizer, self.train_dataloader,  self.meta_dataloader, self.un_train_dataloader, self.demo_train_dataloader
        # )
        self.train_dataloader, self.meta_dataloader, self.un_train_dataloader, self.demo_train_dataloader \
            = self.accelerator.prepare(
            self.train_dataloader, self.meta_dataloader, self.un_train_dataloader,
            self.demo_train_dataloader
        )
        ...
        if self.args.freeze_encoder:
            ...
            model.freeze_lm_encoder()

        if self.args.only_train_bias:
            logger.info("only finetune bias")
            model.freeze_lm_finetune_bias()
        ...
        for epoch in train_iterator:
            ...
            if self.args.demo_condon == 1 and self.global_step > self.args.demo_condon_steps:
                logger.info("######### Start finetuning #########")
                self.args.demo_condon = 0
                self.optimizer = None
                self.lr_scheduler = None

                self.create_optimizer_and_scheduler(t_total)
                self.args.per_device_train_batch_size = self.args.per_device_train_batch_size * 2
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.is_semi == 1 and (delta >= 0):
                    ...
                    if self.args.psuedo_selection_opt == 'meta_st':
                        if delta >= self.args.update_teacher_steps:
                            if delta == self.args.update_teacher_steps:
                    ...
                    if delta == 0:
                        ...
                        if self.args.re_init or self.args.psuedo_selection_opt == 'meta_st':
                            ...
                            self.optimizer = None
                            self.lr_scheduler = None
                            t_total = self.args.update_teacher_steps
                            self.create_optimizer_and_scheduler(num_training_steps=t_total)
                            self.t_total = t_total
                            #####
                            ...
                            #####
                            # model.lm_model, self.optimizer = self.accelerator.prepare(
                            #     model.lm_model, self.optimizer
                            # )
                ...
                if un_inputs is not None and (not finetune):
                    ...
                    tr_loss += self.training_step(model,  un_inputs=un_inputs)

                    tr_loss += self.training_step(model, inputs=inputs)
                elif demo_inputs is not None and self.args.demo_condon == 1:
                    tr_loss += self.demon_condensation_step(model, demo_inputs=demo_inputs, inputs=inputs)
                else:
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    metrics = None
                    if self.args.use_last_epoch and not (self.args.psuedo_selection_opt == 'meta_st' and self.args.is_semi == 1):
                        continue
